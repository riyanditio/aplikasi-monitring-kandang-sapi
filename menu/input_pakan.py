...
def tampilkan_menu_pakan(df_sapi, STRUKTUR_KANDANG, save_data, add_activity_log, user_name, read_sheet_to_df, write_df_to_sheet):
    ...
    with tab1:
        ...
        # 3. Jenis Pakan
        jenis_pakan = st.text_input("3. Jenis / Nama Formula Pakan", placeholder="Contoh: Konsentrat Hijau, Silase, Jerami Fermentasi").strip()
        ...
        if st.button("🚀 Simpan Pemberian Pakan Baru", type="primary", use_container_width=True):
            if not jenis_pakan or pakan_per_ekor <= 0:
                st.error("❌ Gagal Simpan! Jenis pakan wajib diisi dan kuantiti per ekor harus lebih besar dari 0 kg.")
            else:
