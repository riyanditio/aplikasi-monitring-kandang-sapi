...
# Tambahkan parameter user_role di akhir fungsi (default-nya 'operator')
def tampilkan_menu_registrasi(df_sapi, list_jenis_sapi, struktur_kandang, save_data, add_activity_log, user_name, user_role="operator"):
    ...
    # Membuat 2 Tab terpisah agar tampilan tetap rapi dan scannable
    ...
    # ==================== TAB 2: EDIT / HAPUS (DENGAN OTORISASI) ====================
    with tab_edit_hapus:
        st.markdown("### 🔐 Panel Koreksi Data Registrasi")
        
        # 1. Logika Cek Otorisasi Password
        is_authorized = False
        
        if str(user_role).lower() == "admin":
            is_authorized = True
            st.info("👋 Halo Admin! Anda memiliki akses penuh tanpa perlu input password.")
        else:
            st.warning("⚠️ Akun Operator mendeteksi tindakan sensitif. Diperlukan konfirmasi password admin.")
            # Silakan ganti "admin123" di bawah ini dengan password admin internal kandang Anda
            password_input = st.text_input("Masukkan Password Admin:", type="password", help="Minta admin untuk menginput password ini")
            if password_input == "admin123": 
                is_authorized = True
                st.success("🔒 Akses Terbuka! Otorisasi Admin Valid.")
            elif password_input:
                st.error("❌ Password Salah! Hak akses ditolak.")

        # 2. Jika Lolos Otorisasi, Tampilkan Form Kerja
        if is_authorized:
            if df_sapi.empty:
            ...
            st.markdown("---")
            # Membuat opsi list unik berdasarkan index dataframe, kode tiba, dan rfid kandang
            ...
            with sub_tab_edit:
                with st.form("form_edit_registrasi"):
                    ...
                    btn_simpan_edit = st.form_submit_button("Simpan Perubahan", type="primary", use_container_width=True)
                    
                    if btn_simpan_edit:
                        df_sapi.at[idx_target, "Kode Tiba"] = e_kode_tiba
                        df_sapi.at[idx_target, "Kode Sapi"] = e_kode_tiba # Tetap sinkron
                        ...
                        save_data(df_sapi)
                        add_activity_log(user_name, "Edit Registrasi", f"Mengubah data sapi baris {idx_target + 1} menjadi Kelompok {e_kode_tiba} oleh {user_name}")
                        st.success("🎉 Sukses memperbarui data registrasi!")
                        st.rerun()

            # --- SUB-TAB: HAPUS DATA ---
            with sub_tab_hapus:
                st.markdown(f"🚨 **Perhatian:** Tindakan ini akan menghapus data sapi kelompok **{row_sapi['Kode Tiba']}** pada baris ke-{idx_target + 1} secara permanen dari sistem Google Sheets.")
