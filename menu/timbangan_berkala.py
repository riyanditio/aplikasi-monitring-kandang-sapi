...
def tampilkan_menu_timbangan(df_sapi, calculate_adg, save_data, add_activity_log, user_name, read_sheet_to_df, write_df_to_sheet):
    ...
    with tab_edit:
        ...
        if df_riwayat_timbang.empty:
            ...
            df_riwayat_timbang.insert(0, "No Urut", range(1, len(df_riwayat_timbang) + 1))
            st.dataframe(df_riwayat_timbang, use_container_width=True, hide_index=True, column_config={"Bobot (kg)": st.column_config.NumberColumn(format="%.2f"), "ADG (kg/hari)": st.column_config.NumberColumn(format="%.2f")})
            ...
            if btn_col1.button("✏️ Simpan Perubahan", type="primary", use_container_width=True):
                if pwd_input != correct_admin_pwd:
                    ...
                    # Buang kolom 'No Urut' sebelum disave
                    df_save = df_riwayat_timbang.drop(columns=["No Urut"])
                    write_df_to_sheet("riwayat_timbangan", df_save, COLS_RIWAYAT_TIMBANG)
            ...
            if btn_col2.button("🗑️ Hapus Baris Permanen", type="secondary", use_container_width=True):
                if pwd_input != correct_admin_pwd:
                    ...
                    df_save = df_riwayat_timbang.drop(index=idx_pilihan).reset_index(drop=True)
                    df_save = df_save.drop(columns=["No Urut"], errors='ignore')
                    write_df_to_sheet("riwayat_timbangan", df_save, COLS_RIWAYAT_TIMBANG)
    ...
    with tab_analisis:
        ...
        if sapi_analisis and not df_riwayat_timbang.empty:
            ...
            if df_hist.empty:
                ...
                # Buat tabel desimal
                st.dataframe(df_hist.drop(columns=["No Urut"], errors='ignore'), use_container_width=True, hide_index=True, column_config={"Bobot (kg)": st.column_config.NumberColumn(format="%.2f"), "ADG (kg/hari)": st.column_config.NumberColumn(format="%.2f")})